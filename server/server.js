...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/api/health") {
      return send(res, 200, { ok: true, product: "CareFlow", database: "careflow_triage_cases" });
    }
    ...
    if (url.pathname === "/api/records") {
      ...
      if (req.method === "POST") {
        ...
        const record = {
          id: row.id || `careflow-api-${Date.now()}`,
          title: row.title || "Clinical case",
          status: row.status || "Acuity scoring",
          owner: row.owner || "Command nurse",
...
server.listen(port, () => {
  console.log(`CareFlow command center running at http://localhost:${port}`);
});
